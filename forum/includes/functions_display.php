...
function display_forums($root_data = '', $display_moderators = true, $return_moderators = false)
{
	...
	foreach ($forum_rows as $row)
	{
		...
		$template->assign_block_vars('forumrow', array(
			...
			'FORUM_NAME'			=> $row['forum_name'],
			'FORUM_DESC'			=> generate_text_for_display($row['forum_desc'], $row['forum_desc_uid'], $row['forum_desc_bitfield'], $row['forum_desc_options']),
			'TOPICS'				=> ($row['forum_op_only_view'] && !$auth->acl_get('f_op_only_view', $forum_id)) ? '--' : $row['forum_topics'],
			$l_post_click_count		=> ($row['forum_op_only_view'] && !$auth->acl_get('f_op_only_view', $forum_id)) ? '--' : $post_click_count,
			...
			'LAST_POST_TIME'		=> ($row['forum_op_only_view'] && !$auth->acl_get('f_op_only_view', $forum_id)) ? '--' : $last_post_time,
	...
	$template->assign_vars(array(
		...
		'LAST_POST_IMG'		=> ($row['forum_op_only_view'] && !$auth->acl_get('f_op_only_view', $forum_id)) ? '' : $user->img('icon_topic_latest', 'VIEW_LATEST_POST'),
